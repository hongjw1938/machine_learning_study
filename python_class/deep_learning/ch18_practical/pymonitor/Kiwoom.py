...
class Kiwoom(QAxWidget):
    ...
    def _receive_tr_data(self, scr_no, rqname, tr_code, record_name, pre_next,
                         unused1, unused2, unused3, unused4):
        ret = {'scr_no': scr_no, 'rq_name': rqname, 'tr_code': tr_code,
        ...
        if rqname == 'opt10081_req':
            self._opt10081(rqname, tr_code)
        elif rqname == 'opw00001_req':
            self._opw00001(rqname, tr_code)
        elif rqname == "opw00018_req":
            self._opw00018(rqname, tr_code)
    ...
    def send_order(self, rqname, screen_no, acc_no, order_type, code, quantity, price, hoga, order_no):
        self.dynamicCall("SendOrder(String, QString, QString, int, QString, int, int, QString, Qstring)",
    ...
    ##입력문자열에 대해 lstrip으로 - 또는 0 을 제거
    @staticmethod
    def change_format(data):
        strip_data = data.lstrip('-0')
        if strip_data == '' or strip_data == '.00' or strip_data == '.0':
            strip_data = '0'
